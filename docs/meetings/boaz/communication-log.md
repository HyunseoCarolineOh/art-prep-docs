...
| 5 | 오현서 / 천시원 | 시드 학원 리스트 추가 | academies 2개(미지정·테스트미술학원)만 존재. **시드 작품 27개 전부 academy_id=null → 합격작 100장 수집 시 학원정보까지 같이 입력해야 옵션 A 채워짐** |
| 6 | 오현서 | PRD 06 §8 NEEDS CLARIFICATION → BOAZ 영향도 회신 받기 | Phase 2 진입 전 확정 |
| 8 | 지수 | Looker 대시보드 공유 추가 — `ohscaroh555@gmail.com` 계정에 권한 부여 | 6/7 천시원 요청, 오현서 계정 전달 완료 |
| 9 | 오현서 | **데이터 라벨링 작업 완료** | **due 2026-06-19**. 실체 = AI 학습용 전처리 4 Task ([가이드 정리본](./boaz-preprocessing-guide.md)). 6/11 킥오프 완료·기준 확정, 전수 작업 진행 중 |
| 10 | 오현서 | 대시보드 + GA 함께 검토 → 오류·수정사항·피드백 회신 | 6/8 검토 완료(아래 2026-06-08 섹션) → 회신 전달만 남음 |
| 11 | 천시원 | 옵션 B(콘텐츠 운영 인사이트) 보드 빌드 | Supabase DB 권한 확보 후 진행 (대기) |
| 12 | 오현서 | 시드 데이터 정리 | **결정 완료**: "테스트"(id 68) 삭제 처리. 시드 27개는 배포 D-1까지 유지 → 합격작 100장으로 교체 확정. (가입퍼널 보드는 #13으로 이관) |
| 13 | 천시원 | **4p 가입 퍼널/가입완료 차트 Looker 연결 수정** — GA4엔 signup_start·step1·complete 각 2건 존재하나 보드는 No data/빈 차트 | 6/8 검토 발견. 데이터 정상, 그림 연결 문제 |
| 14 | 천시원 | **DAU/WAU/MAU 카드 집계 점검** — 일별 합(SUM)으로 보임(DAU 60 = 일별 막대 합), 활성 사용자 고유 카운트로 정정 검토 | 6/8 검토 발견 |
| 15 | 오현서 / 천시원 | **login·search 이벤트 0건 원인 확인** — 28일간 GA4 수집 0. 코드 미발화 vs 미사용 구분 | 6/8 검토 발견. login 0은 트래킹 누락 의심 |
| 16 | 오현서 / 천시원 | **옵션 A 선행 정리** — PRD 06 현행화 + academies 스키마 확정(제안 풀스펙 vs 현 축소판) + 합격작 academy_id 입력 규칙 | 6/9 발견. #6(§8 회신)과 통합 |
| 17 | 천시원 / 오현서 | **옵션 B** — 결정: **GA `search` 이벤트 수정 먼저**(#15와 묶음) → GA 기반 콘텐츠 분석. Supabase 로그테이블·권한은 후순위 | 6/9 결정. search 발화 정상화 후 옵션 B 재개 |
...
| 7 | GA4/Looker 보드 빌드 — PART 1 4종 + 옵션 C 빌드 완료, 측정기준 10종 등록, 유저 타입 자동 첨부(PR #4) 머지, 배포 사이트 실시간 수집 검증 완료. 조회수 TOP 10 정상 작동 | 2026-06-07 |

---

## 2026-06-11 (목) — 데이터 라벨링(전처리) 킥오프 미팅

> 자료: [회의 시나리오](./labeling-kickoff-2026-06-11.md) · [전처리 가이드 정리본](./boaz-preprocessing-guide.md)

### 배경
- 천시원이 Drive 폴더로 "Artprep팀 전처리 가이드" 전달 → Open #9 라벨링의 실체 확인: vision encoder 학습용 데이터셋 전처리 (①재크롭 ②중복 제거 ③미완성 제거 ④특성 문구 xlsx)
- 회의 전 사전 작업 완료 공유: 전수 인벤토리(크롭 2,982 · 원본 2,811), 중복 클러스터 377개 분석, AI 파일럿(클러스터 20 판정 + 크롭 검수 100장 + 문구 20개)

### 확정 사항
1. **겹침 워터마크·로고 보존**: 작품 위에 겹쳐 인쇄돼 크롭으로 제거 불가한 워터마크는 보존. 작품 바깥 글자·배너만 재크롭 대상
2. **크롭 안 된 원본 326장**: ArtPrep이 크롭 추가 (작업 범위에 포함)
3. **원본 없는 크롭 2장**: 현상 유지
4. **특성 문구 작성 규칙 확정**: 객관 기술만(주관 감상·가치판단 배제), 쉬운 단어, 한 작품 한 문장, 끝은 작품 종류로 마무리. 필수 요소 = 오브제·색·질감 / 구도·시점·배치 축 / 화면 운영(여백·원근·대비). 상세 규칙·예시는 [가이드 정리본](./boaz-preprocessing-guide.md) 참조 → 파일럿 문구 20개는 새 규칙으로 재생성
5. 제거분(중복·미완성)은 삭제 대신 `제거대상` 폴더 이동, 일정·분담은 계획대로 (6/15 중간 체크인, 6/19 전달)

---

## 2026-06-08 (월) — 대시보드 + GA4 교차검증 (Open #10 검토)

> 오현서가 Looker 대시보드 5개 페이지 + GA4 이벤트 보고서(기간 5/11~6/7, 최근 28일)를 함께 검토.
> Looker는 "그림", GA4는 "데이터 원본". 두 개를 대조해 *그림 문제 vs 데이터 문제*를 구분함.

### GA4 원본 이벤트 수집 현황 (28일, 12종 전체)

| 이벤트 | 건수 | 사용자 | 메모 |
|--------|------|--------|------|
| page_view | 312 | 60 | 5/30 191건 스파이크 포함 |
| session_start | 78 | 60 | |
| first_visit | 60 | 60 | = 신규 60 |
| artwork_view | 48 | 23 | 정상 |
| scroll | 48 | 36 | 자동수집 |
| user_engagement | 35 | 25 | 자동수집 |
| filter_use | 24 | 10 | 정상 |
| artwork_save | **3** | 2 | 절대량 부족 |
| form_start | 2 | 2 | 자동수집 |
| signup_complete | **2** | 2 | 수집 정상 |
| signup_start | **2** | 2 | 수집 정상 |
| signup_step1_complete | **2** | 2 | 수집 정상 |

- **PRD 12종 중 6종이 0건**: `artwork_unsave / artwork_upload / artwork_edit / artwork_delete / search / login`
  (목록에 아예 없음 = 28일간 한 번도 발화 안 됨)

### 검토 발견 (증상 → 원인)

1. **5/30 단일 스파이크가 전 지표 지배** — GA4 자체 인사이트도 이상치 자동 감지: "5/30 page_view 191건(예상 9건), 홈('/') 주간 5→119, Safari +75건". 요일=토요일, 시간=10AM 집중. → **데모/외부유입 추정**, 베이스라인 분석 시 제외 검토.
2. **4p 가입완료 No data + 가입 퍼널 빈 차트 → 대시보드 연결 문제 (천시원 수정, Open #13)**. GA4엔 signup 3종 각 2건 존재. 데이터는 정상, Looker가 안 그림.
3. **2p 저장수 TOP 10 No data → 데이터 부족**. artwork_save 3건뿐이라 작품별 표가 안 채워짐. (코드 정상)
4. **1p DAU/WAU/MAU가 합계(SUM)로 보임 → 집계 점검 (Open #14)**. DAU 60 = 일별 막대 합. 고유 활성 사용자로 정정 검토.
5. **login·search 0건 → 미발화/미사용 확인 (Open #15)**. login 0은 트래킹 누락 의심.
6. **표시/가독성**: ① 1p 평균 세션·페이지뷰 + 유저타입 분포 누락(PRD PART1-1) ② 2p "조회수 TOP 10"에 조회수 수치 컬럼 없음(제목만) ③ 1p 일활성·5p 요일 막대가 시간/요일 순이 아닌 값·알파벳순 정렬(요일 Fri·Mon·Sat…) ④ 3p 필터표 `academyType` 값에 "현역"(학생구분 값) 혼입 — 측정기준 매핑 확인.
7. **시드 데이터 왜곡**: 조회 TOP 1위 "테스트", 학원 "미지정" 다수 → 시드 정리(Open #5/#12)와 연결.

### 액션 결과
- Open #10 검토 완료 → 천시원 회신 전달만 남음
- 신규 Open #13(가입 퍼널 연결), #14(DAU 집계), #15(login/search 0건) 추가

### 후속 — 천시원 "대기 중" 4항목 처리 점검 (6/9)

천시원 6/7 "3) 대기 중" 항목별 실제 상태 (Supabase·GA·PRD 06 대조):

| 대기 항목 | 천시원이 말한 blocker | 실제 blocker (오늘 확인) | 처리 |
|-----------|----------------------|--------------------------|------|
| 시드 정리 | 기획 결정 | — | ✅ 완료 (id 68 삭제, 27개 교체방침) |
| 가입 퍼널 보드 | 추가예정 | 데이터는 GA에 있음 | ✅ 천시원 Looker 연결(#13)로 경로 확정 |
| 옵션 A 학원별 | 프론트 페이지+데이터 누적 | ① PRD 06 제안 vs 실제 academies 스키마 불일치 ② 작품 27개 academy_id 전부 null ③ academies에 실제 학원 0개 ④ PRD 06 §0.2 현황 outdated | ⏸ 선행 정리 필요 (#16) |
| 옵션 B 콘텐츠 | Supabase 권한 | 권한 줘도 원본 없음 — search 이벤트 GA 0건 + Supabase 검색/이벤트 로그 테이블 부재. 쓸 수 있는 건 filter_use(24)·tags뿐 | ⏸ 데이터 소스부터 결정 필요 (#17) |

- **신규 Open #16(옵션 A)**: PRD 06 현행화 + academies 스키마 확정(제안 풀스펙 vs 현 축소판) + 합격작 수집 시 academy_id 입력 규칙. §8 영향도 회신(#6)과 통합.
- **신규 Open #17(옵션 B)**: 콘텐츠 분석 데이터 소스 결정 — (a) GA `search` 이벤트 발화 수정 후 GA 기반 (b) Supabase에 검색/이벤트 로그 테이블 신설. 결정 전까지 Supabase 권한 부여는 무의미.

### 후속 — 시드 데이터 점검·정리 (Supabase 직접 조회, 6/9)
- `artworks` 실제 28개 확인 (※ `list_tables` 행수는 추정치라 부정확했음 — 실 카운트로 검증)
  - 시드 27개: `2026-04-09` 일괄 삽입, `user_id=null`, **`academy_id` 전부 null**
  - 테스트 1개: id 68 "테스트"(`2026-05-28`, 실유저 업로드, 오늘 조회 1위 왜곡 주범)
- **id 68 삭제 처리** — 딸린 saved_items 1·ai_reports 1 동반 삭제(트랜잭션). 결과 artworks 27 / saved 6 / reports 2
- **시드 27개**: 5/17 합의대로 배포 D-1까지 유지 → 합격작 100장으로 교체 확정 (Open #12)
- 발견: 시드 `academy_id`/`university` 미입력 다수 → 학원별 보드(옵션 A)는 시드로 불가, 합격작 수집 시 학원·대학 메타 동반 입력 필요 (Open #5)

---

## 2026-06-07 (일) — GA 트래킹 + Looker 대시보드 MVP 완료 공유

### 천시원 → 그룹방 (카톡) [완료] GA 트래킹 + 대시보드 작업 마무리

> 안녕하세요! 초기 MVP까지 논의했던 내용 모두 완료되서 공유드립니다!

**1) GA4 이벤트 트래킹**
- 이벤트 12종 + 측정기준 10종 등록 완료
- 유저 타입 전체 이벤트 자동 첨부 (PR #4)
- 배포 사이트 실시간 수집 검증 완료 (artwork_save · artwork_view · 유저 타입 정상)

**2) Looker 대시보드**
- 보드 4종(DAU / 인기작품 / 대학·전형별 / 유입·가입퍼널) + 옵션 C(시간대) 구축
- 조회수 TOP 10 등 정상 작동 확인
- 일부 지표(저장수 등)는 GA4 처리 지연(24~48h)으로 시간 지나면 자동 반영

**3) 대기 중**
- 옵션 A(학원별 분석): 프론트 학원 선택 페이지 완성 → 데이터 누적 후 가능 (DB 권한과 별개)
- 옵션 B(콘텐츠 분석): Supabase DB 권한 확보 후 진행
- 시드 데이터 정리: 기획 결정 후 실행
- 가입 퍼널 대시보드 추가예정

> Gmail 하나씩 보내주시면 Looker 대시보드에 추가하겠습니다. 추가로 요청드린 데이터 라벨링 작업은 어느정도 진행되었는지 확인 부탁.

### 오현서 → 천시원 (카톡 회신)

1. Looker 공유 계정: `ohscaroh555@gmail.com`
2. 3번 대기 중 항목이 구체적으로 무엇을 대기 중인 작업들인지 질문
3. 데이터 라벨링 작업은 아직 착수 전 — due date 있으면 그 날짜까지 작업하겠다

### 천시원 → 그룹방 (카톡 회신)

- **대기 항목 설명**: ① 프론트 작업이 필요한 항목(옵션 A) ② 배포 이후 작업을 진행할 항목(옵션 B)
  ③ 논의가 필요한 항목(시드데이터 정리 / 가입퍼널 대시보드 추가)
- 데이터 라벨링 작업 **19일(6/19)까지** 부탁 가능 여부 문의

### 오현서 (확정)

- 데이터 라벨링 작업 **6/19까지 완료**로 확정 (학기 중 + 창업 대회 일정으로 5월까지 유의미한 진행 못함, 19일까지 완료 후 전달) → Open #9

### 천시원 → 그룹방

- @지수 Looker 대시보드 공유 추가 부탁 (Open #8)
- @오현서 대시보드 + GA 함께 보면서 오류·수정사항·피드백 부탁 (Open #10)
- 금일 기준 돌다 오류 없이 작동 중 → 추가/수정 위주로 피드백 요청
